...
const TARGET_COURSES = [
  ...
  'excel-essencial',
  'figma',
  'gestao-de-performance-e-cultura-de-resultados',
...
async function main() {
  console.log('=== Fresh URL Downloader - Targeted 27 Courses ===\n');
  ...
  for (const mf of manifestFiles) {
    ...
    if (!TARGET_COURSES.includes(courseSlug)) continue;
  ...
  console.log(`Target courses: ${TARGET_COURSES.length}`);
  console.log(`Total unique lesson pages to visit: ${allLessonUrls.length}\n`);
  ...
  fs.writeFileSync('storage/audit/fresh_url_targeted_27_results.json', JSON.stringify({
    downloaded,
    alreadyExists,
    failed,
    skipped,
